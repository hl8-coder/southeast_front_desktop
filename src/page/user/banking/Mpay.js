    ...
    return <>
      <div className={styles["panel-content"]}>
        ...
        <div className={styles.content}>
          ...
          <div className={styles.row}>
            <div className={styles.label}>{this.props.user.userInfo.currency === "THB" ? 2 : 3}. {intl.get("MPAY_LABEL_PHONE_NUMBER")} <span className={styles.hl}>*</span></div>
            <div className={styles["form-elm"]}>
              <Field className={styles["form-elm"]} value={this.state.related_no} disabled />
            </div>
          </div>
          <div className={styles.row}>
            <div className={styles.label}>{this.props.user.userInfo.currency === "THB" ? 3 : 4}. {intl.get("MPAY_LABEL_METHOD")} <span className={styles.hl}>*</span></div>
            <div className={styles["form-elm"]}>
              {
          ...
          <div className={styles.row + " " + styles["with-link"] + " " + styles.multi}>
            <div className={styles.label}>{this.props.user.userInfo.currency === "THB" ? 4 : 5}. {intl.get("MPAY_LABEL_NUMBER", {type: this.state.name})} <span className={styles.hl}>*</span></div>
            <div className={styles["form-elm"]}>
              ...
              <Link to={`/${window.location.pathname.split("/")[1]}/user/details`}><div className={styles.link}>{intl.get("LINK_MANAGE_MPAY", {type: this.state.name})}</div></Link>
            </div>
          </div>
          <div className={styles.row}>
            <div className={styles.label}>{this.props.user.userInfo.currency === "THB" ? 5 : 6}. {this.props.user.userInfo.currency === "THB" ? intl.get("MPAY_LABEL_CODE_1") : intl.get("MPAY_LABEL_CODE")} <span className={styles.hl}>*</span></div>
            <div className={styles["form-elm"]}>
              <Field type="number"　onKeyPress={this.handleKeyPress}　placeholder={this.props.user.userInfo.currency === "THB" ? intl.get("MPAY_PLACEHOLDER_CODE_1") : intl.get("MPAY_PLACEHOLDER_CODE")} className={styles["form-elm"]} onChange={val => this.setState({mpay_trading_code: val})} />
            </div>
            {
              this.props.user.userInfo.currency === "THB" &&
